...
from GUI.tools.filepath_tools import check_file_encoding_mime_type
...
def find_unique_identifier(df: pd.DataFrame) -> str | None:
...
def csv_data_preprocessing(file: str, encoding: str = 'utf-8'):
    """
        Method to preprocess the csv data
        # TODO: check and redo this function
    """
    # We ensure that the encoding is the correct one.
    encoding, _ = check_file_encoding_mime_type(file)
    df = read_csv_file(file, encoding)
